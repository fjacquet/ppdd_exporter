...
- `${VAR:-default}` fallbacks in config env references, ported from `pscale_exporter`.
  ...
  nothing is now rejected, so a stray `PPDD1_PASSWORD=` line fails at startup instead
  of authenticating with an empty credential. The shipped `config.yaml` now uses
  `insecureSkipVerify: "${PPDD1_SKIP_CERTIFICATE:-false}"`, so the setting is env-driven out of the box
